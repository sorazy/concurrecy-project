...
public class UnboundedDequeue<T> {
	...
	final private AtomicReference<NodeHint<Object>> leftNodeHint;
    final private AtomicReference<NodeHint<Object>> rightNodeHint;
		...
		StmUtils.atomic(new Runnable() {
            ...
            public void run() {
				...
				while(true) {
					...
					hintCopy = (NodeHint<T>) leftNodeHint.get();
					edge = findLeftEdge(hintCopy);
					...
					if(edgeIndex != 1) {
						if(in.compareAndSet(inCopy, new DequeueSlot<>(inCopy.data, inCopy.count + 1))
							...
							updateLeftHint(hintCopy, edgeNode, edgeIndex - 1);
						return;
					}
					// edge is straddling or on a boundary
						...
						if(outCopy.data == left_null) {
							...
							if(in.compareAndSet(inCopy, new DequeueSlot<>(inCopy.data, inCopy.count + 1))
								...
								updateLeftHint(hintCopy, newNode, array_size - 2);
								return;
							}
						}
							...
							if(farCopy.data == left_null) {
								if(in.compareAndSet(inCopy, new DequeueSlot<>(inCopy.data, inCopy.count + 1))
									...
									updateLeftHint(hintCopy, outNode, array_size - 2);
									return;
								}
							}
								...
								if(in.compareAndSet(inCopy,  new DequeueSlot<>(inCopy.data, inCopy.count + 1))
									...
									updateLeftHint(hintCopy, edgeNode, 1);
									NodeHint<T> rightHint = findRightEdge((NodeHint<T>) rightNodeHint.get());
									updateRightHint(rightHint, rightHint.buffer, rightHint.loc);
									// retire?
								}
							}
						} // end straddling edge
